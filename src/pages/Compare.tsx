...
export default function Compare() {
    ...
    const consensusRanking = useMemo(() => {
        ...
        // Simple mock: just use the ranking with the highest score as 'Consensus' for now
        // or just the first one. Let's pick the one from 'u1' as the baseline expert.
                    ...
                    <div className="glass-morphism-premium rounded-[2.5rem] border border-white/10 overflow-hidden bg-black/40">
                        ...
                        <div className="grid grid-cols-12 gap-0 border-b border-white/10 bg-white/[0.02]">
                            ...
                            <div className="col-span-5 p-6 bg-white/[0.01]">
                                <div className="flex items-center gap-3">
                                    <div className="w-12 h-12 rounded-2xl bg-white/10 flex items-center justify-center text-white font-black text-xs">VS</div>
                                    <div>
                                        <h4 className="font-black text-white italic uppercase tracking-tighter">Rival List</h4>
                                        <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">
                                            {selectedRivalId === 'CONSENSUS' ? 'Expert Baseline' : `@${USERS.find(u => u.id === rivalRanking?.userId)?.username}`}
                                        </span>
                                    </div>
                                </div>
                            </div>
